...
class New_Start_Final_Xor( object ):
    ...
    def construct_clf( self ):
        ...
        with tf.variable_scope( "nn" ) as scope:
            self.l0_obj = Group_L0( 3, 8, activation = tf.nn.relu, weight_decay = 1.0 , name = "l0" )
            ...
            self.l1_obj = Group_L0( 8, 4, activation = tf.nn.relu, weight_decay = 1.0, name = "l1" )
            ...
            self.logit_train, _, self.final_w, _ =  dense( self.concat_train, 2, initializer = tf.random_normal_initializer( stddev = 1./tf.sqrt( 10./2. ) ), name = "logit" )
    ...
    def construct_loss( self ):
        ...
        self.classifier_loss = self.classifier_loss \
                        -0.1*( tf.reduce_mean( self.l0_regu ) + tf.reduce_mean( self.l1_regu ) )
