...
class Baseline_Xor( object ):
    ...
    def construct_clf( self ):
        ...
        with tf.variable_scope( "nn" ) as scope:
            self.l0, self.l0_pre, self.w_l0, self.b_l0 = dense( self.in_sample, 16, 
                                                                activation = tf.nn.relu,
                                                                initializer = tf.random_normal_initializer( stddev = 1./tf.sqrt( 8./2. ) ), 
                                                                name = "l0" )
            self.l1, self.l1_pre, self.w_l1, self.b_l1 = dense( self.l0, 8, 
                                                                activation = tf.nn.relu,
                                                                initializer = tf.random_normal_initializer( stddev = 1./tf.sqrt( 4./2. ) ), 
                                                                name = "l1" )
