...
class New_Start_Final_Cifar_10( object ):
    ...
    def construct_clf( self ):
        ...
        with tf.variable_scope( "nn" ) as scope:
            self.l0_obj = Group_L0( 3072, 2048, activation = tf.nn.leaky_relu, weight_decay = 1.0 , name = "l0" )
            ...
            self.l1_obj = Group_L0( 2048, 1024, activation = tf.nn.leaky_relu, weight_decay = 1.0, name = "l1" )
            ...
            self.logit_train, _, self.final_w, _ =  dense( self.concat_train, 2, initializer = tf.random_normal_initializer( stddev = 1./tf.sqrt( 2./2. ) ), name = "logit" )
            self.logit_test, _, _, _ =  dense( self.concat_test, 2, initializer = tf.random_normal_initializer( stddev = 1./tf.sqrt( 2./2. ) ), name = "logit", reuse = True )
    ...
    def construct_loss( self ):
        ...
        self.classifier_loss = self.classifier_loss \
                        -1.5*( tf.reduce_mean( self.l0_regu ) + tf.reduce_mean( self.l1_regu ) )
